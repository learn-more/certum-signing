...
.DESCRIPTION
    Logging in wants an account name and a TOTP code from the enrolled phone app. The enrolment is an otpauth://
    URI behind the QR code, so the code the phone would have shown is generated here instead.

    SimplySign Desktop takes both on its command line:

        SimplySignDesktop.exe /autologin <user> <otp>

    The widely copied alternative - driving the login dialog with WScript.Shell SendKeys - needs a window in the
    foreground and keystrokes landing in it, and failed on GitHub's runners with the certificate never appearing.

    Success is the certificate being in the store, not the process still being alive. With a thumbprint known the
    script waits for exactly that certificate, so a failed login fails here rather than later inside signtool.
...
.NOTES
    The TOTP generator follows blinkdisk's .github/scripts/connect-simplysign.ps1, which follows
    https://www.devas.life/how-to-automate-signing-your-windows-app-with-certum/. The login does not, see above.

    /autologin is undocumented. It is read as commandLineArgs[1], the account as [2] and the code as [3], so it
    has to come first and all three have to be present. Confirmed in the shipped binary by the "/autologin"
    literal and the autoLoginUser / autoLoginOtp fields; see SIGNING.md.
#>
[CmdletBinding()]
param(
...
if (-not ('CertumSigning.Totp' -as [type]))
{
    Add-Type -Language CSharp -TypeDefinition @'
using System;
using System.Security.Cryptography;

namespace CertumSigning
{
    public static class Totp
    {
        ...
        // Seconds left in the current step, so the caller can avoid typing a code that expires mid-dialog.
        ...
        // Split out from Now so it can be checked against the RFC 6238 vectors, which are all "at time T".
...
# The generator needs no account or enrolment, and it is the part that fails silently - a wrong code just looks
# like a refused login. RFC 6238 Appendix B, all three algorithms.
if ($SelfTest)
{
    # The RFC's seeds are ASCII strings, Base32-encoded here. Pad the tail rather than dropping it: 20 bytes divide
    # into 5-bit groups exactly, 32 and 64 do not, and a truncated secret fails only the SHA-256/512 vectors.
    ...
    foreach ($row in $vectors)
    {
        ...
        {
            $count++
            $actual = [CertumSigning.Totp]::At($seeds[$case[0]], 8, 30, $case[0], $t)
            $ok = $actual -eq $case[1]
            if (-not $ok) { $failed++ }
    ...
    Write-Host ''
    if ($failed) { throw "$failed of $count RFC 6238 vectors failed - the generated codes are wrong, so no login would be accepted." }
...
if (-not $OtpUri) { throw 'No OTP URI. Set CERTUM_OTP_URI (see SIGNING.md).' }
if (-not $UserId) { throw 'No account name. Set CERTUM_USERNAME (see SIGNING.md).' }

# A secret pasted into the CI settings with a trailing newline surfaces as a refused login, not as whitespace.
$OtpUri = $OtpUri.Trim()
$UserId = $UserId.Trim()

# /autologin reads the account from argv[2] and the code from argv[3], and Start-Process does not quote what it is
# handed. A space in the account shifts the code to argv[4] and logs in with half an account name.
if ($UserId -match '\s')
{
...
# Certum enrols with SHA-256, not the SHA-1 most authenticators default to, so algorithm, digits and period are
# all read from the URI rather than assumed.
$uri = [Uri]$OtpUri
$query = @{}
...
# /autologin sends WM_CLOSE to an instance already running, but a forced stop is surer and leaves nothing holding
...
# A code about to roll over gets checked against the next step. Waiting out a few seconds beats a failed release.
$remaining = [CertumSigning.Totp]::SecondsRemaining($period)
if ($remaining -lt 5)
{
...
$otp = [CertumSigning.Totp]::Now($secret, $digits, $period, $algorithm)
...
# X509Store rather than the Cert: drive: the provider serves a cached view within a process, so a certificate
# arriving after the first poll would stay invisible for the life of the script. Opening and closing the store per
# poll re-reads it; the certificate objects stay valid after the handle closes.
...
# value can carry a newline or BOM. None survive a comparison against what the store reports.
$wanted = ($Thumbprint -replace '[^0-9A-Fa-f]', '').ToUpperInvariant()
$CodeSigningOid = '1.3.6.1.5.5.7.3.3'

# The real test: SimplySign registers the cloud certificate into the user store once the session is up, and until
# it is there signing cannot work no matter how healthy the process looks.
...
while ((Get-Date) -lt $deadline)
{
    ...
    {
        # Match the OID, not the friendly name - the latter is localised.
...
if (-not $found)
{
    # Thumbprints are public, and this separates the two cases that otherwise look identical: nothing arrived (the
    # login was refused), or something did and CERTUM_CERTIFICATE_SHA1 names a different certificate. LocalMachine
    # is listed too - a certificate registered there is invisible to signtool's default lookup, so it reads as "no
    # certificate" while sitting in plain view in certmgr.
    ...
    throw "SimplySign is running but $what never showed up in the user store. Either the login was refused - check CERTUM_USERNAME and that the runner's clock is not skewed against the TOTP step - or CERTUM_CERTIFICATE_SHA1 does not match any certificate listed above."
}
